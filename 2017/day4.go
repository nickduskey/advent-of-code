...
func p2(input []string) int {
	...
	for rowCounter, row := range input {
		...
		for i := 0; i < len(strs); i++ {
			for j := 0; j < len(strs); j++ {
				...
				str1 := sortStr(strs[i])
				str2 := sortStr(strs[j])
				val := isAnagram(str1, str2)
				if val == true {
					badRows = append(badRows, rowCounter)
				}
			}
		}
	}
	...
	for k := range badRows {
		if seen[badRows[k]] == true {
			// Do nothing
		} else {
			seen[badRows[k]] = true
			dedupedRows = append(dedupedRows, badRows[k])
		}
	}
	fmt.Println(dedupedRows)
