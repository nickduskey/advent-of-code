...
func prepareInput() [][]int {
	raw := `104	240	147	246	123	175	372	71	116	230	260	118	202	270	277	292
	...
	for _, line := range lines {
		...
		preparedInput = append(preparedInput, row)
	}
...
// Day2P1 is Day 2 part 1's function
func Day2P1(rows [][]int) int {
	var diffs []int
...
func evenlyDivisible(slice []int) int {
	divisors := []int{}
	for i := 0; i < len(slice); i++ {
		for j := 0; j < len(slice); j++ {
			...
			if slice[j]%slice[i] == 0 {
				divisors = append(divisors, slice[j])
				divisors = append(divisors, slice[i])
			}
		}
	}
	quotient := divisors[0] / divisors[1]
	return quotient
}

// Day2P2 is Day 2 part 2's function
func Day2P2(rows [][]int) int {
	var remainders []int
	for _, row := range rows {
...
func Day2() {
	...
	diffs := Day2P1(input)
	r := Day2P2(input)
	fmt.Println("Part 1 solution:", diffs)
	fmt.Println("Part 2 solution:", r)
}
